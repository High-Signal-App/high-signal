...
class AnnotationTests(unittest.TestCase):
    def test_detects_regional_pressure(self) -> None:
        ...
        self.assertEqual(out.classifierVersion, "semantic-rules-v2.1")
        self.assertEqual(out.model, "none")
        self.assertFalse(out.llm)
        self.assertGreater(out.intentScore, 0)
        self.assertEqual(out.intentConfidence, "high")
        self.assertLess(out.sentimentPolarity, 0)
        self.assertGreater(out.evidenceDensity, 0)
        self.assertGreater(out.signalStrength, 0)
