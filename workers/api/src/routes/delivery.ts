...
function userIdFromHeaders(c: {
  req: { header: (k: string) => string | undefined };
}): string | null {
  return c.req.header('X-Clerk-User-Id') ?? null;
}

function emailFromHeaders(c: {
  req: { header: (k: string) => string | undefined };
}): string | null {
...
deliveryRoute.get('/log', async (c) => {
  ...
  try {
    const rows = await db(c.env.DB)
      ...
      .where(and(eq(schema.deliveryLog.userId, userId), gte(schema.deliveryLog.createdAt, since)))
...
deliveryRoute.post('/test', async (c) => {
  ...
  if (!transport.ready) {
    console.warn(
      `[delivery] test send refused — email transport unconfigured (${transport.reason})`
    );
...
deliveryRoute.post('/retry/:logId', async (c) => {
  ...
  try {
    const rows = await db(c.env.DB)
      ...
      .from(schema.deliveryLog)
      .where(
        and(eq(schema.deliveryLog.id, c.req.param('logId')), eq(schema.deliveryLog.userId, userId))
      )
      .limit(1);
    const row = rows[0];
    ...
    await db(c.env.DB)
      ...
      .where(and(eq(schema.deliveryLog.id, row.id), eq(schema.deliveryLog.userId, userId)));
...
deliveryRoute.on(['GET', 'POST'], '/unsubscribe', async (c) => {
  ...
  if (!secret) {
    console.warn(
      '[delivery] unsubscribe refused — no DELIVERY_UNSUBSCRIBE_SECRET/ADMIN_TOKEN set (fail closed)'
    );
...
export async function runDeliveryWindow(
  ...
  if (!transport.ready && !dryRun) {
    console.warn(
      `[delivery] email transport unconfigured (${transport.reason}); skipping run (fail closed)`
    );
    return { skipped_run: transport.reason };
  }

  let prefs: (typeof schema.deliveryPreferences.$inferSelect)[];
  try {
  ...
  for (const p of prefs) {
    ...
    if (
      prior &&
      prior.status === 'failed' &&
      !isAutomaticRetryEligible(prior.attempt, prior.nextAttemptAt?.getTime() ?? null, now)
    ) {
      bump('skipped');
      continue;
    }
    ...
    const newAttempt = prior ? (prior.status === 'failed' ? prior.attempt + 1 : prior.attempt) : 1;
    ...
    if (result.ok) {
      ...
      if (await hasThreeConsecutiveFailures(env.DB, p.userId, p.channel)) {
        ...
        console.warn(
          `[delivery] auto-disabled email channel for ${p.userId} after 3 consecutive failures`
        );
        bump('auto_disabled');
      }
    }
...
async function hasThreeConsecutiveFailures(
  ...
  const recent = await db(d1)
    ...
    .from(schema.deliveryLog)
    .where(and(eq(schema.deliveryLog.userId, userId), eq(schema.deliveryLog.channel, channel)))
    .orderBy(desc(schema.deliveryLog.briefDate))
    .limit(3);
  // Rows come newest-first; shouldAutoDisable expects oldest-first.
  return shouldAutoDisable(
    recent.map((r) => r.status as 'failed' | 'sent' | 'skipped' | 'queued').reverse()
  );
}
...
async function recordSkip(
  ...
  await db(d1)
    ...
    .onConflictDoNothing({
      target: [schema.deliveryLog.userId, schema.deliveryLog.channel, schema.deliveryLog.briefDate],
    });
}
...
async function composeBriefSnapshot(
  ...
  if (!env.API_BASE) {
    console.error(
      '[delivery] composeBriefSnapshot called without API_BASE; cron will skip every user'
    );
    return null;
  }
