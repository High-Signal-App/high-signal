...
- **Startups / demand:** **Product Hunt** (`producthunt.py`, keyless RSS — new launches; avoids the OAuth API), **Google Trends** (`google_trends.py`, keyless daily-trends RSS — the demand-side signal for the new-ideas domain; noisy by nature, curated downstream). 35 / 20 events.
...
- **Markets:** Polymarket, Manifold, Kalshi, Metaculus (optional token) — probabilities only, not equity prices; **CoinGecko** (`coingecko.py`, keyless) — crypto trending coins + 24h movers (fills the zero crypto-coverage gap). 15 events.
