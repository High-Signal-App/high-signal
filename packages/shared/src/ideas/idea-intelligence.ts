...
const OPPORTUNITY_THEMES = [
  {
    ...
    nextStep:
      'Run audits for five owned products and turn every missing proof area into a concrete page/task.',
  },
  {
    ...
    nextStep:
      'Collect app-builder complaints from communities and ship a manual weekly teardown before building tooling.',
  },
  {
    ...
    targetUser:
      'solo builders and product teams deciding what small tools or features to build next',
    complaintPattern:
      'Users repeatedly describe the same missing workflow in different words before a category is obvious.',
    nextStep:
      'Track two communities for one week and only promote clusters with repeated pain plus a clear buyer/user.',
  },
  {
    id: 'local-control',
    ...
    nextStep:
      'Validate whether the control need is strong enough for paid usage or only a self-hosted feature.',
  },
  {
  ...
  },
  {
    ...
    targetUser:
      'local service businesses, retailers, ecommerce sellers, restaurants, and solo operators',
    complaintPattern:
  ...
  },
  {
    ...
    targetUser:
      'builders choosing consumer, local, marketplace, media, or SMB-support product bets',
    complaintPattern:
  ...
  },
  {
    ...
    targetUser:
      'builders deciding whether a product idea works in a specific city, country, or customer segment',
    complaintPattern:
...
export function communityDigestEvidenceQuality(
  digest: CommunityDigestSnapshot
): IdeaEvidenceQuality {
  const rawSummary = digest.summary as
    | (CommunityDigestSnapshot['summary'] & {
        key_trend?: CommunityDigestSnapshot['summary'] extends null
          ? never
          : NonNullable<CommunityDigestSnapshot['summary']>['keyTrend'];
        ...
        key_action?: CommunityDigestSnapshot['summary'] extends null
          ? never
          : NonNullable<CommunityDigestSnapshot['summary']>['keyAction'];
      })
    | null;
  const keyTrend = rawSummary?.keyTrend ?? rawSummary?.key_trend;
  const notableDiscussions =
    rawSummary?.notableDiscussions ?? rawSummary?.notable_discussions ?? [];
  ...
  const noiseFlags = NOISY_COMMUNITY_PATTERNS.filter(({ pattern }) => pattern.test(text)).map(
    ({ flag }) => flag
  );
  ...
  const genericRisk: IdeaEvidenceGenericRisk =
    noiseFlags.includes('thin-source-count') ||
    noiseFlags.includes('no-product-intent-repeat') ||
    noiseFlags.length >= 3
      ? 'high'
...
function sourceIdentity(evidence: IdeaFlowEvidence) {
  ...
  if (evidence.href.startsWith('/markets'))
    return `market:${evidence.href.split('#')[1] ?? 'watchlist'}`;
  try {
...
export function analyzeIdeaAgainstFlow(idea: string, evidence: IdeaFlowEvidence[]): IdeaAnalysis {
  ...
  const demandScore = clampScore(
    20 + supporting.length * 16 + (containsAny(joinedSupport, DEMAND_WORDS) ? 18 : 0)
  );
  const timingScore = clampScore(
    20 + classified.length * 8 + (containsAny(joinedAll, TIMING_WORDS) ? 24 : 0)
  );
  const evidenceScore = clampScore(classified.length * 14 + supporting.length * 10);
  const riskScore = clampScore(
    contradicting.length * 22 + (containsAny(joinedAll, RISK_WORDS) ? 22 : 0)
  );
  const fitScore = clampScore(
    demandScore * 0.36 + timingScore * 0.28 + evidenceScore * 0.26 - riskScore * 0.18
  );
...
export function generateProductOpportunities(evidence: IdeaFlowEvidence[]): ProductOpportunity[] {
  ...
  return OPPORTUNITY_THEMES.map((theme) => {
    const matched = usable
      .filter((item) =>
        theme.signalLayer === 'market-watch' ? item.source === 'market' : item.source !== 'market'
      )
      .map((item) => ({
        item,
    ...
    const highConfidenceSourceCount = matched.filter(
      (item) => !item.quality && item.confidence === 'high'
    ).length;
    ...
    const confidence: 'low' | 'medium' | 'high' =
      ...
      matched.some((item) => item.confidence === 'high')
        ...
        : hasCrossSourceSupport &&
            matched.length >= 2 &&
            (repeatedSignalCount >= 4 || highConfidenceSourceCount >= 2)
          ? 'medium'
          : 'low';
