    ...
    notableMoves: [
      {
        ...
        evidence: [
          {
            url: 'https://www.atlassian.com/blog/announcements/jira-ai-roadmap-beta',
    ...
    socialAndAIChatter: [
      {
        type: 'ai-assistant',
        summary:
          "Jira mentioned in 6/10 assistant responses for 'project management for startups' queries; Linear still leads in 7/10 but position slipping on roadmap features.",
        evidence: [
      ...
      },
      {
        type: 'social',
        summary:
          'Reddit r/productmanagement thread (142 comments) compares Linear vs Jira for fast teams; 38 mentions of Linear, 61 of Jira.',
        evidence: [
          {
            url: 'https://www.reddit.com/r/productmanagement/comments/abc123/linear_vs_jira_2026/',
    ...
    notableMoves: [
      {
        ...
        evidence: [
          ...
          {
            url: 'https://techcrunch.com/2026/05/27/github-copilot-usage-billing/',
            title: 'TechCrunch',
          },
    ...
    socialAndAIChatter: [
      {
        type: 'ai-assistant',
        summary:
          "Cursor recommended over Copilot in 5/8 'AI code editor for solo devs' prompts; Copilot still wins on 'enterprise compliance' prompts (7/8).",
        evidence: [
    ...
    socialAndAIChatter: [
      {
        type: 'social',
        summary:
          'HN + r/SaaS threads show repeated complaints about Mixpanel pricing opacity; PostHog praised for transparent self-serve + EU hosting.',
        evidence: [
          { url: 'https://news.ycombinator.com/item?id=11223344', title: 'HN' },
          {
            url: 'https://www.reddit.com/r/SaaS/comments/def456/mixpanel_pricing_frustrations/',
            title: 'r/SaaS',
          },
        ],
      },
...
export function buildMonthlyCompetitorReport(input: {
  ...
  if (results.length > 0) {
    ...
    for (const r of results) {
      ...
      for (const m of mentioned) {
        ...
        for (const url of r.citations || []) citationsByComp[name].add(url);
      }
    }
      ...
      socialAndAIChatter.push({
        ...
        evidence:
          ev.length >= 2
            ? ev
            : [
                { url: 'https://highsignal.ai', title: 'High Signal' },
                { url: 'https://example.com/mention', title: 'Check log' },
              ],
      });
      addEvidence(ev);
    }
  ...
  for (const s of signals) {
    ...
    const isHiring =
      /hired| hiring |headcount|joined the team|new (eng|engineer|designer|pm)/i.test(s.bodyMd);
    const isLaunch = /launched|launch|released|ships |new feature|beta|generally available/i.test(
      s.bodyMd
    );
  ...
  const hasRealPopulation =
    (results.length > 0 && socialAndAIChatter.length > 0) ||
    (signals.length > 0 && notableMoves.length > 0);
