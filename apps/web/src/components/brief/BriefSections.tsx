...
function stateCopy(state: BriefCategoryState, empty: string) {
  if (state.status === 'unavailable') {
    return 'This category could not be composed from the live source store. No substitute content was inserted. Try again later or check Sources for coverage health.';
  }
  return empty;
}
...
function SectionShell({
  ...
  return (
    <section id={id} className="scroll-mt-20 border-b border-[var(--color-line)] py-10 first:pt-8">
      ...
      {state.status === 'ready' && count > 0 ? (
        <div className="mt-6 border-t border-[var(--color-line)]">{children}</div>
      ) : (
        <div className="mt-6 border-y border-dashed border-[var(--color-line)] py-8" role="status">
...
function StockItem({ item }: { item: BriefStockItem }) {
  return (
    <article className="grid gap-6 border-b border-[var(--color-line)] py-7 last:border-b-0 md:grid-cols-[minmax(0,1fr)_190px]">
      <div>
        ...
        <Link
          ...
          className="mt-3 block max-w-4xl text-2xl font-medium leading-8 tracking-[-0.02em] text-[var(--color-fg)] hover:text-[var(--color-accent)]"
        >
          {item.headline}
        </Link>
...
function IdeaItem({ item }: { item: BriefIdeaItem }) {
  return (
    <article className="border-b border-[var(--color-line)] py-7 last:border-b-0">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_190px]">
        <div>
          ...
          <h3 className="mt-3 max-w-4xl text-2xl font-medium leading-8 tracking-[-0.02em]">
            {item.title}
          </h3>
          <p className="mt-3 max-w-[70ch] text-sm leading-6 text-[var(--color-muted)]">
            {item.description}
