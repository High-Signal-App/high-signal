  ...
  return (
    <>
      ...
      <PageShell max="max-w-4xl">
        ...
        <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-x-2 gap-y-1 font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--color-muted)]">
          <Link className="min-h-11 py-3 hover:text-[var(--color-accent)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-accent)]" href="/">
            High Signal
          </Link>
          <span aria-hidden="true">/</span>
          <Link className="min-h-11 py-3 hover:text-[var(--color-accent)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-accent)]" href="/explore">
            Intelligence guides
          </Link>
          <span aria-hidden="true">/</span>
        ...
        <section className="mt-8 border-y border-[var(--color-line)]" aria-labelledby={`${guide.key}-evidence`}>
          <div className="grid gap-2 py-4 sm:grid-cols-[minmax(12rem,0.65fr)_minmax(0,1.6fr)] sm:gap-10">
            ...
            <p className="text-sm leading-6 text-[var(--color-muted)]">
              Public facts used on this page. Open any row to inspect the corresponding High Signal surface.
            </p>
          </div>
        ...
        <div className="divide-y divide-[var(--color-line)] border-b border-[var(--color-line)]">
          {guide.sections.map((section) => (
            <section key={section.title} className="grid gap-4 py-8 md:grid-cols-[minmax(12rem,0.65fr)_minmax(0,1.6fr)] md:gap-10">
              <h2 className="text-xl font-medium tracking-tight text-[var(--color-fg)]">
                {section.title}
              </h2>
              <div className="space-y-4">
                {section.paragraphs.map((paragraph) => (
                  <p key={paragraph} className="max-w-[72ch] text-sm leading-7 text-[var(--color-muted)]">
                    {paragraph}
                  </p>
                ))}
              </div>
