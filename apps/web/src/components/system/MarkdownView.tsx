...
function inline(text: string): ReactNode[] {
  ...
  while ((match = pattern.exec(text))) {
    ...
    if (link) {
      ...
      nodes.push(
        <code className="border border-zinc-800 bg-zinc-950 px-1 py-0.5 font-mono text-[0.9em] text-zinc-200" key={key}>
          {token.slice(1, -1)}
        </code>,
      );
    } else if (token.startsWith("**")) {
    ...
    <div className="space-y-5 text-sm leading-7 text-zinc-400">
      {blocks.map((block, index) => {
        if (block.type === "heading") {
          ...
          return (
            <Tag className="pt-2 text-lg font-medium leading-snug tracking-tight text-zinc-100" key={index}>
              {inline(block.text)}
            </Tag>
          );
        }
        ...
        if (block.type === "quote") {
          return (
            <blockquote className="border-l border-[var(--color-accent)] pl-4 text-zinc-300" key={index}>
        ...
        if (block.type === "list") {
          ...
          return (
            <Tag className={`${block.ordered ? "list-decimal" : "list-disc"} space-y-2 pl-5`} key={index}>
        ...
        if (block.type === "code") {
          return (
            <pre className="overflow-x-auto border border-zinc-800 bg-black p-4 font-mono text-xs leading-6 text-zinc-300" key={index}>
                  ...
                  {block.rows.map((row, rowIndex) => (
                    <tr key={rowIndex}>
                      {row.map((cell, cellIndex) => (
                        <td className="border-b border-zinc-900 px-3 py-2 align-top" key={cellIndex}>
                          {inline(cell)}
                        </td>
                      ))}
                    </tr>
