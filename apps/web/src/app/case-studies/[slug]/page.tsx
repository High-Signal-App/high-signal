...
import { BreadcrumbJsonLd, CompanyProfileJsonLd } from '@/components/seo/structured-data';
import { fetchJson } from '@/lib/api';
import { SITE_URL } from '@/lib/site';
import { evaluateCompany, robotsForVerdict } from '../../../../public-corpus-policy.mjs';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const verdict = evaluateCompany(study);
  const category = study.category === 'Other' ? 'company' : study.category;
  const description = study.description
    ? `${study.description} Explore official provenance and meaningfully similar companies.`
    : `${study.name} company profile with official-source provenance and similar companies.`;
  return {
    title: `${study.name}: ${category}, provenance, and similar companies`,
  ...
  return (
    <PageShell max="max-w-5xl">
      <BreadcrumbJsonLd
        trail={[
          { name: 'Home', path: '/' },
          { name: 'Company universe', path: '/case-studies' },
      ...
      {verdict.eligible && (
        <CompanyProfileJsonLd
          ...
          category={study.category}
          sourceUrls={study.sourceEvidence.map((item) => item.sourceUrl)}
          peerNames={similarityCluster.map(({ company }) => company.name)}
        />
      )}
      ...
      <section className="mt-12 grid gap-5 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <Panel eyebrow="source evidence" title="Why this company exists here">
          <div className="mt-5 space-y-4">
            {study.sourceEvidence.length === 0 && !verdict.eligible && (
