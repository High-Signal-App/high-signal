  ...
  return (
    <PageShell>
      ...
      <SectionHeader eyebrow="market context" title="Equities Snapshot">
        Daily returns, volatility, and 52-week context across the working universe of
        US, international, ETF, index, and crypto tickers. Refreshed nightly after US close.
      </SectionHeader>
      ...
      <Panel eyebrow="filter" title="Filter">
        <form method="get" action="/equities" className="flex flex-wrap gap-3 text-sm">
          ...
          <FilterSelect name="assetClass" label="Asset class" value={params["assetClass"]} options={assetClasses} />
            ...
            <table className="w-full min-w-[1100px] border-collapse text-sm">
              <thead className="text-xs uppercase tracking-wide text-[var(--color-muted)]">
                <tr className="border-b border-zinc-800">
                  <Th label="Ticker" thisKey="ticker" sortKey={sortKey} dir={sortDir} preserve={preserve} />
                  <Th label="Name" thisKey="name" sortKey={sortKey} dir={sortDir} preserve={preserve} />
                  <Th label="Sector" thisKey="sector" sortKey={sortKey} dir={sortDir} preserve={preserve} />
                  <Th label="Country" thisKey="country" sortKey={sortKey} dir={sortDir} preserve={preserve} />
                  <Th label="Last" thisKey="last_close" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="1d" thisKey="ret_1d" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="30d" thisKey="ret_30d" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="90d" thisKey="ret_90d" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="1y" thisKey="ret_1y" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="5y" thisKey="ret_5y" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="Vol30" thisKey="volatility_30d" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="vs 52wH" thisKey="dist_to_52w_high" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="MaxDD1y" thisKey="max_drawdown_1y" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                  <Th label="β" thisKey="beta_vs_spy" sortKey={sortKey} dir={sortDir} preserve={preserve} align="right" />
                </tr>
              </thead>
              <tbody>
                {rows.map((r) => (
                  <tr key={r.ticker} className="border-b border-zinc-900 hover:bg-zinc-900/50">
                    ...
                    <td className="py-1.5 pr-3 text-right font-mono text-zinc-200">{formatPrice(r.last_close)}</td>
                    <td className={`py-1.5 pr-3 text-right font-mono ${moveTone(r.ret_1d)}`}>{formatPct(r.ret_1d)}</td>
                    <td className={`py-1.5 pr-3 text-right font-mono ${moveTone(r.ret_30d)}`}>{formatPct(r.ret_30d)}</td>
                    <td className={`py-1.5 pr-3 text-right font-mono ${moveTone(r.ret_90d)}`}>{formatPct(r.ret_90d)}</td>
                    <td className={`py-1.5 pr-3 text-right font-mono ${moveTone(r.ret_1y)}`}>{formatPct(r.ret_1y)}</td>
                    <td className={`py-1.5 pr-3 text-right font-mono ${moveTone(r.ret_5y)}`}>{formatPct(r.ret_5y)}</td>
                    <td className="py-1.5 pr-3 text-right font-mono text-zinc-400">{formatPct(r.volatility_30d, 0)}</td>
                    <td className={`py-1.5 pr-3 text-right font-mono ${moveTone(r.dist_to_52w_high)}`}>{formatPct(r.dist_to_52w_high)}</td>
                    <td className={`py-1.5 pr-3 text-right font-mono ${moveTone(r.max_drawdown_1y)}`}>{formatPct(r.max_drawdown_1y)}</td>
...
function Th({
  ...
  return (
    <th className={`py-2 pr-3 font-medium ${align === "right" ? "text-right" : "text-left"}`}>
      <a
        href={sortLink(sortKey, thisKey, dir, preserve)}
