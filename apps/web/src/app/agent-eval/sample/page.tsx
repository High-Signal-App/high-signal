  ...
  linear: {
    ...
    gaps: [
      ...
      },
      {
        ...
        promptText: "Is there a cheaper option than Linear that still handles Jira's core use case?",
        observed:
    ...
    gaps: [
      {
        ...
        promptText: "What tools do engineering leaders recommend for issue tracking instead of Mixpanel?",
        observed:
...
export default async function SampleReportPage({
  ...
  return (
    <PageShell max="max-w-5xl">
      ...
      >
        How {brand} actually shows up when buyers ask ChatGPT, Claude, Gemini, and
        Perplexity for {sample.input.buyerMission}. Read the gaps, then run the same
        audit on your own brand.
      </SectionHeader>
      ...
      <section className="mt-10 grid gap-8 md:grid-cols-[1.05fr_0.95fr]">
        <Panel eyebrow="brand visibility" title="By AI platform">
          <p className="mt-3 text-sm leading-6 text-[var(--color-muted)]">
            Mention rate = how often {brand} appears in the response. Recommendation
            rate = how often {brand} is the answer the agent leads with.
          </p>
        ...
        <Panel eyebrow="share of voice" title="Brand vs competitors">
          <p className="mt-3 text-sm leading-6 text-[var(--color-muted)]">
            How prompt responses split between {brand} and the named competitor set
            across all 15 prompts and 4 platforms.
          </p>
          <div className="mt-5 divide-y divide-[var(--color-line)] border-y border-[var(--color-line)]">
            {report.competitors
              .slice()
      ...
      <section className="mt-12 border-t border-[var(--color-line)] pt-6">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--color-muted)]">
          Each gap is a prompt where the agent did not surface {brand} as the
          recommended answer. Severity reflects how close this prompt sits to a real
          buying decision.
        </p>
      ...
      <section className="mt-12 border-t border-[var(--color-line)] pt-8 sm:flex sm:items-baseline sm:justify-between sm:gap-6">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-[var(--color-muted)]">
            ...
            deterministic synthesis otherwise. Output is a scored audit with prompts,
            evidence gaps, and the fixes ranked by impact.
          </p>
        </div>
        <Link
      ...
      <p className="mt-10 border-l-2 border-[var(--color-line)] pl-3 font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--color-muted)]">
        Sample report. Numbers above are illustrative of a real run on the same
        15-prompt matrix — not a live audit. Real audits cite the verbatim response
        per platform.
      </p>
    </PageShell>
  );
