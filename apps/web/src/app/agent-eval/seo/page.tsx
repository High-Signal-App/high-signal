...
export default async function SeoAuditPage({
  ...
  const orderedChecks = report
    ? [...report.checks].sort((left, right) => CHECK_PRIORITY[left.status] - CHECK_PRIORITY[right.status])
    : [];
  ...
  return (
    <>
      ...
      {report && !error ? (
        <>
          ...
          {highestLeverageCheck ? (
            <section className="mt-8 border-y border-[var(--color-line)] py-5" aria-labelledby="highest-leverage-fix">
              <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--color-accent)]">
                highest-leverage fix
              </p>
              <h2 id="highest-leverage-fix" className="mt-3 text-xl font-medium tracking-tight text-[var(--color-fg)]">
          ...
          <section className="mt-10 border-t border-[var(--color-line)]" aria-labelledby="technical-checks">
            <h2 id="technical-checks" className="py-4 font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--color-accent)]">
      ...
      <section className="mt-14 border-t border-[var(--color-line)]" aria-label="Understanding the audit">
