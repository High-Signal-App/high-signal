  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-14 sm:px-6 sm:py-16">
      ...
      {data.sectors.length === 0 ? (
        ...
        <section className="mt-8 max-w-full overflow-x-auto" aria-label="Sector signal ledger">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {s.topEntities.slice(0, 5).map((e) => (
                          <a
                            key={e}
                            href={`/entities/${e}`}
                            className="inline-flex min-h-11 items-center border border-zinc-800 px-2 font-mono text-[10px] text-zinc-300 hover:border-zinc-600 hover:text-white sm:min-h-0 sm:px-1.5 sm:py-0.5"
                          >
                            {e}
                          </a>
                        ))}
    ...
    <div className="mt-4 flex flex-wrap gap-2 font-mono text-[10px] uppercase tracking-[0.18em]">
      {opts.map((d) => (
        <a
          key={d}
          href={`?days=${d}`}
          className={`inline-flex min-h-11 items-center border px-3 sm:min-h-0 sm:px-2 sm:py-0.5 ${
            d === active
