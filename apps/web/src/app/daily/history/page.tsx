...
function safeCategory(value?: string): SignalContentCategory | '' {
  return CATEGORY_LABELS.some((item) => item.value === value)
    ? (value as SignalContentCategory)
    : '';
}
    ...
    <main className="mx-auto max-w-5xl px-6 py-16">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500">
              {summary.from} to {summary.to} / {summary.daysReturned} day
              {summary.daysReturned === 1 ? '' : 's'} / {summary.totals.requirements} requirement
              {summary.totals.requirements === 1 ? '' : 's'}
            </p>
          </div>
          <a
        ...
        <div className="mt-4 grid gap-4 text-xs leading-6 text-zinc-500 md:grid-cols-3">
          <div>
            ...
            <div className="mt-1 font-mono">
              {summary.totals.requirements} requirement / {summary.totals.broadInsights} read
              coverage / {percent(summary.totals.productRequirements, summary.totals.broadInsights)}
            </div>
          </div>
          <div>
            <div className="font-mono uppercase tracking-[0.18em] text-zinc-600">repeats</div>
            <div className="mt-1 font-mono">
              {summary.totals.repeatedSignalCount} repeated source signals
            </div>
          </div>
          <div>
            <div className="font-mono uppercase tracking-[0.18em] text-zinc-600">range export</div>
            <a
              className="mt-1 block font-mono text-[var(--color-accent)] hover:underline"
              href={`/daily/range.json?${rangeQuery}`}
            >
              /daily/range.json
            </a>
            ...
            <article key={day.date} className="py-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-3 font-mono text-[10px] uppercase tracking-[0.18em]">
                  <a
                    className="text-zinc-500 hover:text-[var(--color-accent)]"
                    href={`/daily?${dayQuery}`}
                  >
                    open daily
                  </a>
                  <a
                    className="text-zinc-500 hover:text-[var(--color-accent)]"
                    href={`/daily/tasks?${tasksQuery}`}
                  >
                    tasks
                  </a>
                  <a
                    className="text-zinc-500 hover:text-[var(--color-accent)]"
                    href={`/daily/tasks.json?${tasksQuery}`}
                  >
                    json
                  </a>
                  <a
                    className="text-zinc-500 hover:text-[var(--color-accent)]"
                    href={`/daily/sources?date=${day.date}`}
                  >
                    source audit
                  </a>
                </div>
      ...
      {summary.days.length === 0 ? (
        <p className="mt-10 text-sm text-zinc-500">
          No daily source-read snapshots matched this range.
        </p>
      ) : null}
    </main>
  );
