...
function evidenceFromRefreshes(records: ProductFlowRefreshRecord[]): IdeaFlowEvidence[] {
  return latestRefreshRecords(records)
    ...
    .map((record) => {
      const sourceKey =
        record.sourceId ?? record.subreddit ?? record.target ?? record.label ?? record.source;
...
export default async function PersonalPage({
  ...
  const selectedReport =
    reports.find((report) => report.date === requestedDate) ?? reports[0] ?? null;
  ...
  const sourceReadDate =
    resolveAcceptedRefreshDate(
      refreshes,
      requestedSourceDate ?? requestedDate ?? selectedReport?.date
  ...
  const sourceReadDomains = countByValues(
    sourceReadsAll.flatMap((item) => item.annotation.domains)
  );
  ...
  const sourceReadRequirementTypes = countByValues(
    sourceReads.map((item) => item.annotation.requirementType)
  );
  const sourceReadQualityGates = countByValues(
    sourceReads.map((item) => item.annotation.qualityGate.status)
  );
  ...
  const pendingSync = brief.actionTasks.filter(
    (item) => item.status === 'todo' && item.syncStatus !== 'created'
  ).length;
    ...
    <PageShell max="max-w-5xl">
      ...
      <StatGrid
        items={[
          {
            label: 'Decision',
            value: topAction ? topAction.action : 'none',
            sub: topAction?.productName ?? 'no recommendation',
          },
          {
            label: 'Confidence',
            value: `${brief.usefulnessAudit.score}/100`,
            sub: brief.usefulnessAudit.readiness,
          },
          {
            label: 'Freshness',
            value:
              brief.freshness.evidenceAgeDays === null
                ? '?'
                : `${brief.freshness.evidenceAgeDays}d`,
            sub: 'latest evidence age',
          },
          {
            label: 'Actions',
            value: brief.recommendations.length.toString(),
            sub: 'build / change / watch',
          },
          {
            label: 'Evidence mix',
            value: `${brief.evidenceBreakdown.worldChange}/${brief.evidenceBreakdown.appComplaint}/${brief.evidenceBreakdown.marketWatch}`,
            sub: 'world / complaints / markets',
          },
      ...
      <section className="mt-10 grid gap-px border border-[var(--color-line)] bg-[var(--color-line)] md:grid-cols-4">
        {[
          [
            '1',
            'Decision',
            topAction ? `${topAction.action} / ${topAction.productName}` : 'No ranked action yet',
          ],
          [
            '2',
            'Reason',
            topAction?.whyNow ?? 'Need more evidence before a recommendation is useful',
          ],
          [
            '3',
            'Proof',
            topAction
              ? `${topAction.evidence.length} linked source${topAction.evidence.length === 1 ? '' : 's'}`
              : 'no evidence',
          ],
      ...
      <section className="mt-10 border-y border-[var(--color-line)] py-6">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--color-muted)]">
          ...
          {marketSnapshot.latestRefreshAt
            ? ` / ${marketSnapshot.latestRefreshAt.slice(0, 16).replace('T', ' ')} UTC`
            : ''}
          ...
          <details className="mt-4 border-t border-[var(--color-line)] pt-4">
            ...
            <details className="mt-4">
              ...
              <p className="mt-3 text-xs leading-6 text-[var(--color-muted)]">
                ...
                annotation pass. Hugging Face enrichment is optional and off by default. Automation
                is {automationStatus.workflow} at {automationStatus.schedule}, currently{' '}
            ...
            <form
              action="/personal"
              className="mt-5 grid gap-3 border-y border-[var(--color-line)] py-4 md:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_1fr_1fr_auto]"
            >
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
              <div className="border border-[var(--color-line)] p-4">
                ...
                <div className="mt-3 break-words font-mono text-xs leading-6 text-[var(--color-fg)]">
                  {sourceQualityAudit.acceptedSnapshots} accepted /{' '}
                  {sourceQualityAudit.rejectedSnapshots} rejected /{' '}
              ...
              <div className="border border-[var(--color-line)] p-4">
                ...
                <div className="mt-3 break-words font-mono text-xs leading-6 text-[var(--color-fg)]">
                  {sourceQualityAudit.rejectedReasons
            ...
            <div className="mt-4 flex flex-wrap gap-4 font-mono text-[10px] uppercase tracking-[0.18em]">
              ...
              <a
                className="text-[var(--color-muted)] hover:text-[var(--color-accent)]"
                href={`/daily/sources.json?date=${sourceReadDate}`}
              >
                sources json
              </a>
            </div>
                ...
                <div className="mt-3 divide-y divide-[var(--color-line)]">
                  {sourceQualityAudit.actions.slice(0, 4).map((action) => (
                    <div key={action.title} className="py-4">
                      ...
                      <div className="mt-2 text-sm leading-6 text-[var(--color-fg)]">
                        {action.title}
                      </div>
                ...
                <div className="mt-4 divide-y divide-[var(--color-line)] border-y border-[var(--color-line)]">
                  {requirementQueue.map((item) => (
                    ...
                    >
                      ...
                      {item.taskDraft ? (
                        <div className="mt-3 font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--color-muted)]">
                          task draft / {item.taskDraft.saasMakerProjectSlug} /{' '}
                          {item.taskDraft.status} / {item.taskDraft.priority}
                        </div>
                      ) : null}
                      <div className="mt-2 font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--color-muted)]">
        ...
        <section className="mt-6 grid gap-8">
          {brief.recommendations.map((item) => (
            <Panel
              ...
              title={
                <span id={item.id}>
                  <span className={actionTone(item.action)}>{item.action}</span> /{' '}
                  {item.productName}
                </span>
              }
            >
      ...
      >
        <form
          action="/personal"
          className="mt-5 flex flex-col gap-3 border-y border-[var(--color-line)] py-4 md:flex-row"
        >
          ...
          <>
            <MetricGrid
              items={[
                {
                  label: 'generated',
                  value: selectedReport.generatedAt?.slice(0, 10) ?? 'unknown',
                },
                ...
                { label: 'recommendations', value: selectedReport.recommendations ?? 'unknown' },
                {
                  ...
                  value: selectedReport.latestEvidence?.slice(0, 10) ?? 'unknown',
                },
              ]}
            />
