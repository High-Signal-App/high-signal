...
export default async function DataSourcePage({
  ...
  const allHistoryRequested = all === '1';
  const requestedDate = validIsoDay(date);

  let sourceLive: DataSourceLive | null = null;
  try {
    const summary = await readDataSources();
    sourceLive = summary.sources.find((item) => item.id === source) ?? null;
  } catch {
    /* Detail endpoint below remains independently usable. */
  }

  const latestObservedDate = sourceLive?.latestObservedAt
    ? fmtDate(sourceLive.latestObservedAt)
    : undefined;
  const selectedDate = allHistoryRequested ? undefined : (requestedDate ?? latestObservedDate);
  const allHistory = allHistoryRequested || !selectedDate;
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-12">
      ...
      <header className="mt-4 mb-8 border-b border-zinc-800 pb-5">
        ...
        <div className="mt-3 flex flex-wrap gap-x-6 gap-y-1 font-mono text-xs tabular-nums text-zinc-400">
          ...
          {entry && (
            <>
              ...
              <span>{entry.windowDays}d fetch window</span>
              <span>cadence: {entry.cadence.replace('_', ' ')}</span>
        ...
        {entry && (
          <div className="mt-2 space-y-1 text-xs text-zinc-500">
            <p>
              Extracts: <span className="text-zinc-400">{entry.keeps}</span> ·{' '}
              <span className="text-zinc-400">{entry.contentDepth}</span>
            </p>
            <p>Retention: {entry.retention}</p>
            <p>Access basis: {entry.accessBasis}</p>
          </div>
        )}
        {sourceLive && (
          <p className="mt-2 font-mono text-[10px] text-zinc-500">
            Last ingested: {fmtDate(sourceLive.lastIngestedAt)} · run status:{' '}
            {sourceLive.runStatus.replaceAll('_', ' ')}
            {sourceLive.futureCount > 0
              ? ` · ${sourceLive.futureCount.toLocaleString()} future-effective observations retained`
              : ''}
          </p>
        )}
      ...
      {data?.available && events.length === 0 && (
        <p className="text-sm text-zinc-500">
          {selectedDate
            ? `No events stored for this source on ${selectedDate}. Try adjacent days or all history.`
            : 'No observations are stored for this configured source. This does not prove the adapter failed; consult its run status and cadence above.'}
        </p>
      )}
