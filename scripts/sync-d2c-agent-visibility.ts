...
function main() {
  ...
  proc.on('close', (code) => {
    if (code) {
      console.error(
        '[d2c:sync-av] remote D1 write failed. Review the Wrangler error above; authorization failures require CLOUDFLARE_API_TOKEN to target CLOUDFLARE_ACCOUNT_ID with Account:D1:Edit.'
      );
    }
    process.exit(code ?? 1);
  });
