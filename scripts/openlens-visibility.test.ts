...
console.log('\nclassifyOwnership');
{
  ...
  checkEq('unknown when URL malformed', classifyOwnership('garbage', brand).ownership, 'unknown');
}
...
console.log('\ncomputeShareOfVoice');
{
  ...
  checkEq('competitorX share', sov.competitorShare['competitorX'], 0.5);
...
console.log('\ncomputeTrends — windowed bucketing');
{
  ...
  checkEq("most recent day's mention rate is 0.5", recent.mentionRate, 0.5);
...
async function finish() {
  ...
  checkEq(
    'configuration copy labels category as Topic',
    mentionsPage.includes('label="Topic"'),
    true
  );
  checkEq('check progress uses prompts', mentionsPage.includes('totalQueries} prompts'), true);
  checkEq(
    'post-check refreshes are independent',
    productsRoute.includes('Promise.allSettled(['),
    true
  );
  checkEq(
  ...
  checkEq(
    ...
    /productsRoute\.post\(['"]\/mentions\/:brandId\/report\/share-token['"]/.test(productsRoute),
    true
  );
