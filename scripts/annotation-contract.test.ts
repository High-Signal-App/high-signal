...
assert.equal(tsAnnotations[0]?.classifierVersion, "semantic-rules-v2.1");
assert.equal(tsAnnotations[0]?.intentConfidence, "high");
assert.ok((tsAnnotations[0]?.sentimentPolarity ?? 0) < 0);
assert.ok((tsAnnotations[0]?.signalStrength ?? 0) > 0);
