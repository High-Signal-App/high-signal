...
for (const source of registry.sources) {
  ...
  assert.ok(
    (source.limit ?? 8) >= 4 && (source.limit ?? 8) <= 12,
    `${source.id} should stay concise`
  );
  if (source.type === 'rss')
    assert.match(source.target, /^https:\/\//, `${source.id} rss target must be https`);
}

const byType = countBy(registry.sources.map((source) => source.type));
assert.ok(
  (byType.get('reddit') ?? 0) >= 35,
  'reddit coverage should include broad public/community sources'
);
...
assert.ok(
  (byClass.get('ai-dev') ?? 0) >= 8,
  'AI/developer workflow coverage should remain present'
);
assert.ok(
  (byClass.get('startup-builder') ?? 0) >= 5,
  'startup-builder coverage should remain present'
);
assert.ok(
  (byClass.get('small-business') ?? 0) >= 12,
  'small-business coverage should remain present'
);
assert.ok(
  (byClass.get('public-consumer') ?? 0) >= 6,
  'public-consumer coverage should remain present'
);
assert.ok((byClass.get('regional') ?? 0) >= 8, 'regional coverage should remain present');
assert.ok(
  (byClass.get('platform-primary') ?? 0) >= 12,
  'primary platform/news coverage should remain present'
);
