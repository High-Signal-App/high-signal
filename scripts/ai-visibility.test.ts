...
console.log('resolvePlatforms');
{
  ...
  check(
    'labels include chatgpt/perplexity/gemini/claude',
    ['chatgpt', 'perplexity', 'gemini', 'claude'].every((p) => labels.includes(p as never))
  );
  check(
    'perplexity is marked grounded',
    multi.find((p) => p.platform === 'perplexity')?.grounded === true
  );
  ...
  check(
    'no provider keys → single custom fallback',
    fallback.length === 1 && fallback[0]!.platform === 'custom'
  );
  ...
  check(
    'brand endpoint adds a custom column on top of providers',
    withBrand.some((p) => p.platform === 'custom') &&
      withBrand.some((p) => p.platform === 'chatgpt')
  );
}
...
console.log('\nmention-judge');
{
  ...
  check(
    'judge prompt names the brand and competitor',
    prompt.includes('Acme') && prompt.includes('Globex')
  );
  check('judge prompt demands JSON-only', /Return ONLY a JSON object/i.test(prompt));

  check(
    'extractJsonObject strips a code fence',
    extractJsonObject('```json\n{"a":1}\n```') === '{"a":1}'
  );
  check(
    'extractJsonObject finds a balanced object amid prose',
    extractJsonObject('Here you go: {"x": {"y": 2}} done') === '{"x": {"y": 2}}'
  );
  check(
    'extractJsonObject returns null when no object',
    extractJsonObject('no json here') === null
  );
  ...
  check(
    'verdict parses core fields',
    verdict?.brandMentioned === true &&
      verdict?.brandRecommended === true &&
      verdict?.brandSentiment === 'positive'
  );
  check('verdict dedupes citations', verdict?.citations.length === 1);
  check(
    'verdict backfills omitted competitor (Initech) as not-mentioned',
    verdict?.competitorsMentioned.some((c) => c.name === 'Initech' && !c.mentioned) === true
  );
  check(
    'malformed judge output → null (caller falls back)',
    parseMentionVerdict('totally not json', []) === null
  );
  check(
    'not-mentioned brand forces null sentiment',
    parseMentionVerdict(JSON.stringify({ brandMentioned: false, brandSentiment: 'positive' }), [])
      ?.brandSentiment === null
  );
}
...
console.log('\nvisibility analytics');
{
  ...
  check(
    'per-platform mention rate: gemini 0, chatgpt 1',
    perPlatform['gemini'] === 0 && perPlatform['chatgpt'] === 1
  );
  ...
  check(
    'consistency = 2 of 3 platforms mention brand',
    Math.abs(score.components.consistency - 2 / 3) < 1e-9
  );
  ...
  check(
    'citation gaps rank g2.com top (cited twice)',
    gaps[0]?.host === 'g2.com' && gaps[0]?.citations === 2
  );
  check(
    'competitor host classified as competitor',
    gaps.some((g) => g.host === 'competitor.com' && g.ownership === 'competitor')
  );
  ...
  check(
    'report carries score + recommendations',
    report.score.score === score.score && report.recommendations.length > 0
  );
  check(
    'report flags cross-engine gap (absent on gemini)',
    report.recommendations.some((r) => r.area === 'consistency')
  );
}
...
console.log('\nrecommendations');
{
  const lowVis = buildVisibilityRecommendations({
    score: {
      score: 10,
      grade: 'F',
      components: { mention: 0.1, recommendation: 0, citation: 0, consistency: 0.25 },
      platformsCovered: 1,
      platformsTotal: 4,
    },
    shareOfVoice: {
      ...
      brandRecommendationRate: 0,
      brandCitationRate: 0,
      competitorShare: { Globex: 0.8 },
      citationShare: {},
    },
    perPersona: [],
  ...
  check(
    'competitor out-appearing flagged',
    lowVis.some((r) => r.title.includes('out-appears'))
  );
  ...
  const strong = buildVisibilityRecommendations({
    score: {
      score: 90,
      grade: 'A',
      components: { mention: 0.9, recommendation: 0.8, citation: 0.6, consistency: 1 },
      platformsCovered: 4,
      platformsTotal: 4,
    },
    shareOfVoice: {
      ...
      brandRecommendationRate: 0.8,
      brandCitationRate: 0.6,
      competitorShare: { Globex: 0.2 },
      citationShare: {},
    },
    perPersona: [],
    citationGaps: [],
  ...
  check(
    'strong visibility → single hold-the-line rec',
    strong.length === 1 && strong[0]!.title.includes('hold the line')
  );
}
