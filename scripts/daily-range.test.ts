...
async function main() {
  ...
  assert.ok(dates.length >= 1, 'expected at least one accepted refresh date');

  const products = productGraph.products as PersonalProductProfile[];
  const daysRequested = Math.min(30, dates.length);
  ...
  assert.equal(
    summary.totals.taskExports,
    summary.days.reduce((sum, day) => sum + day.taskExportCount, 0)
  );
  assert.equal(
    summary.totals.productRequirements,
    summary.days.reduce((sum, day) => sum + day.productRequirementCount, 0)
  );
