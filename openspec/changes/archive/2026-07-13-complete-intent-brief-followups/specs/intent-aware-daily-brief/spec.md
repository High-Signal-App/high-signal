...
For an owner-scoped Daily Brief, the system SHALL attach the highest-scoring
open intent opportunity for each connected brand to section 4. The attached
finding SHALL retain its source URL, source title and excerpt, platform, buyer
stage, action type, score, competitor context, and discovery time.
...
The system SHALL translate actionable open intent opportunities into section 5
items. It SHALL retain the source context, SHALL link back to the Mentions
intent inbox, and SHALL avoid duplicating an Agent Eval evidence task backed by
the same source URL.
...
Every rendered Daily Brief surface SHALL expose the source and meaningful intent
context for intent-backed section 4 and section 5 items.
...
The system MUST preserve existing mention, Agent Eval, and watchlist brief output
when intent storage is unavailable, including deployments where migration 0014
...
Migration 0014 SHALL be executable against an isolated local D1 database and
SHALL create the intent table and its expected indexes without requiring remote
