...
Plan 0009 already supplies delivery preference/log tables, an email cron, automatic retries, one-click unsubscribe, and a signed-in settings page. The schema intentionally includes `rss` and `digest_json` channels plus `rss_token`, but only the email channel is functional. The existing `/digest/rss` and web `/digest.json` surfaces publish weekly signal rows and do not represent a user's daily brief. Manual retry is named in the worker API plan but no route or UI action exists.

The change must be local-only: no D1 migration, DNS/provider work, secrets, production configuration, or deployment. It must reuse the existing Clerk proxy trust boundary and current `delivery_preferences` columns.
...
- Applying migrations, configuring Email Routing/DNS, adding secrets, deploying, or sending production mail.
